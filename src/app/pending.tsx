    ...
    const unsubAuth = onAuthStateChanged(auth, (user) => {
      ...
      const unsubDoc = onSnapshot(userRef, (snap) => {
        ...
        if (data.status === "approved") {
          // Admin approved — redirect to dashboard immediately
          router.replace("/dashboard" as any);
        } else if (data.status === "rejected") {
          // Admin rejected — sign out and show message
          signOut(auth);
...
const s = StyleSheet.create({
  ...
  bgGlow: {
    ...
    backgroundColor: C.darkGreen, opacity: 0.3,
  },
  ...
  logoMarkText: { fontSize: 26, fontWeight: "800", color: C.white },
  brand: { fontSize: 13, fontWeight: "700", color: C.gold, letterSpacing: 6 },

  // Pending card
  pendingCard: {
    backgroundColor: C.navyCard, borderWidth: 1,
    borderColor: C.gold, borderRadius: 18,
    padding: 24, marginBottom: 24, alignItems: "center",
  },
  pendingIcon: { fontSize: 40, marginBottom: 14 },
  pendingTitle: { fontSize: 20, fontWeight: "700", color: C.white, marginBottom: 10 },
  pendingDesc: { fontSize: 13, color: C.offWhite, textAlign: "center", lineHeight: 22, marginBottom: 16 },
  pendingBadge: {
    flexDirection: "row", alignItems: "center", gap: 8,
    backgroundColor: "rgba(29,158,117,0.1)", borderWidth: 0.5,
    borderColor: C.forestGreen, borderRadius: 20,
    paddingHorizontal: 14, paddingVertical: 7,
  },
  pendingDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: C.forestGreen },
  pendingBadgeText: { fontSize: 11, color: C.forestGreen, fontWeight: "500" },
  ...
  featureCardLocked: { opacity: 0.5 },
  featureIcon: { fontSize: 20 },
  featureBody: { flex: 1 },
  featureTitle: { fontSize: 14, fontWeight: "600", color: C.white, marginBottom: 2 },
  ...
  nextTitle: { fontSize: 13, fontWeight: "700", color: C.white, marginBottom: 14 },
  ...
  nextNumText: { fontSize: 11, fontWeight: "700", color: C.forestGreen },
  nextText: { fontSize: 13, color: C.offWhite, flex: 1, lineHeight: 20 },
